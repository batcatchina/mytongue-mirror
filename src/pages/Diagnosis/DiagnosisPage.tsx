...
const DiagnosisPage: React.FC = () => {
  ...
  // 处理AI识别结果 - 自动预填充舌象特征
  const handleAIRecognition = (result: AIRecognitionResult) => {
    // 舌色
    if (result.tongue_color?.value) {
      setTongueColor(result.tongue_color.value, result.tongue_color.confidence);
    }
    
    // 舌形
    if (result.tongue_shape?.value) {
      setTongueShape(result.tongue_shape.value, 0.9);
    }
    ...
    if (result.shape_distribution) {
      setShapeDistribution({
        depression: result.shape_distribution.depression || [],
        bulge: result.shape_distribution.bulge || []
      });
    }
    
    toast.success('AI识别完成，请确认舌象特征');
  };
            ...
            <div className="tcm-card p-5">
              ...
              <div className="mb-6">
                <ImageUpload 
                  ...
                  onAIRecognition={handleAIRecognition}
                />
              </div>
