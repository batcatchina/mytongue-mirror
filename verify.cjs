...
const testCases = [
  {
    ...
    test: () => {
      const InferenceChain = require('./src/engine/core/InferenceChain');
      const chain = new InferenceChain.InferenceChain();
  ...
  },
  {
    ...
    test: () => {
      const Layer1Processor = require('./src/engine/layers/Layer1Processor');
      const processor = new Layer1Processor.Layer1Processor();
  ...
  },
  {
    ...
    test: () => {
      const Layer2Processor = require('./src/engine/layers/Layer2Processor');
      const processor = new Layer2Processor.Layer2Processor();
      const output = processor.process({
        tongueAnalysis: {
          bodyColor: '淡红',
          shape: '胖大',
      ...
      return output.summary.label.includes('气虚') && hasAntiIntuitive;
    }
  },
  {
  ...
  },
  {
    ...
    test: () => {
      const Layer1Processor = require('./src/engine/layers/Layer1Processor');
      const Layer2Processor = require('./src/engine/layers/Layer2Processor');
      const layer1 = new Layer1Processor.Layer1Processor();
      const layer2 = new Layer2Processor.Layer2Processor();
